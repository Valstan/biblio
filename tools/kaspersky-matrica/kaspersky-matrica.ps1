...
# Значения по умолчанию: используются, только если ничего не удалось определить с диска.
# Прод-адрес Матрицы (electron-app/src/main/index.ts: MATRICA_API_URL ?? этот адрес).
$DefaultApiBaseUrl = 'https://a6fd55b8e0ae.vps.myjino.ru'
$KnownProdIps      = @('195.161.41.30')   # прод-строка реестра проектов; DNS уточняет
